...
<div class="modal fade bd-example-modal-lg" tabindex="-1" id="reservationForm" role="dialog" aria-labelledby="myLargeModalLabel">
...
<script type="text/javascript">
    ...
    function cinClient (cin){ 
     if(cin.length == 8 ){ 
        $.ajax({
           url : '<?php echo base_url() ?>Client/clientInfo/',
           type : 'POST',
           data: {cin:cin},
           success : function(data){ // success est toujours en place, bien sûr !
             ...
             alert("Remplir la date de CIN ,  la date de naissance et code postal") ;

             $('#dateCin').val(data[0].dateCin).css("border", "5px solid red");
             $('#birth').val(data[0].birthday).css("border", "5px solid red");
            
        
             $('#n').val(data[0].n).css("border", "5px solid green");
             $('#rue').val(data[0].rue).css("border", "5px solid green");
             $('#ville').val(data[0].ville).css("border", "5px solid green");
             $('#codePostal').val(data[0].codePostal).css("border", "5px solid green");

             $('#email').val(data[0].email).css("border", "5px solid green");
             $('#mobile').val(data[0].mobile).css("border", "5px solid green");
             $('#mobile2').val(data[0].mobile2).css("border", "5px solid green");
           },
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
          initialView: '<?php echo $calenderType ?>',
           ...
           headerToolbar: {
            ...
            locale: 'fr', 
             events: [
               ...
               if($calenderType == "dayGridWeek") {  if($i==50){ break; } } } ?>
              ]
        });
        calendar.render();
      });
